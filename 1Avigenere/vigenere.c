 ...
 */
static void process(FILE *input, FILE *output, const char *key, bool decrypt) {
 ...
 */
static bool parseArgs(int argc, char *argv[], bool *decrypt, char **key,
                      char **outfile, int *fileStartIndex) {
  int opt;
...
int main(int argc, char *argv[]) {
  ...
  if (!parseArgs(argc, argv, &decrypt, &key, &outfile, &fileStartIndex)) {
    return EXIT_FAILURE;
  }
  ...
  if (fileStartIndex >= argc) { // No file arguments, use stdin
    process(stdin, output, key, decrypt);
  } else {
    for (int i = fileStartIndex; i < argc; i++) {
      ...
      process(input, output, key, decrypt);
      fclose(input);
    }
  }
